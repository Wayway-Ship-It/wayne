  ...
  <div class="supplier-list">
    <el-card shadow="hover">
      <div slot="header" class="clearfix">
        <span>供应商管理</span>
        <el-button type="primary" @click="openAddDialog" style="float: right;">添加供应商</el-button>
      </div>
    ...
    <!-- 添加/编辑对话框 -->
    <el-dialog title="供应商信息" :visible.sync="dialogVisible" width="600px">
...
export default {
  ...
  methods: {
    loadData() {
      this.$axios.get('/api/supplier/list', { params: this.searchForm })
        .then(res => {
          if (res.code === 200) {
    ...
    saveSupplier() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$axios.post('/api/supplier/save', this.form)
            .then(res => {
              if (res.code === 200) {
    ...
    deleteSupplier(id) {
      this.$confirm('确定要删除这个供应商吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$axios.delete(`/api/supplier/delete/${id}`)
          .then(res => {
            if (res.code === 200) {
